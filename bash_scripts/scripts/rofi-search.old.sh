#!/usr/bin/env bash
#
# Rofi script for searching the web and local files using baloo indexer
#
# Requirements:
# I found this script from github and incorporated local search
# And now at time of uploading I can't seem to find the original script 
...
CACHE_DIR="$HOME/.cache/rofi/rofi-scripts/web-search"
BROWSER="firefox"
...
{
query=$1
baloosearch "$query" |sort |uniq >/tmp/search.txt
...
handle_query()
{
    ...
    if [ "${SITES[$site]}" = "localsearch" ];then
	    local "$query"
    else	
