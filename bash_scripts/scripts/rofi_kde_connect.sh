...
SendKeys(){
    output="?"
    TEMPFILE=/tmp/VimFloat > $TEMPFILE
    urxvtc -t "vim-anywhere" -n 'popup' -e "${EDITOR:-vi}" -c 'startinsert' $TEMPFILE
    xclip -i -sel clip < $TEMPFILE
    output=$(xclip -o -sel clip)
    notify-send "$output"
...
Notification_menu () {
    ...
    menu=$(echo $options | rofi -dmenu -i -p "$prompt" -l $optionNum )
    case "$menu" in
	*'View' ) 
	    ticker1=`qdbus org.kde.kdeconnect /modules/kdeconnect/devices/$2/notifications/$1 org.kde.kdeconnect.device.notifications.notification.ticker`
	    notify-send "$ticker1";;
...
#    fi
    options+=$(printf "\\nRefresh\\n")
    menu=$(echo $options | rofi -dmenu -i -p $1 -l $optionNum )
    case "$menu" in
	*'Ping') 
	    message=$(echo 'Ping' | rofi -dmenu -i -p "Msg to send")
	...
	*'Send File') 
        ...
        $TERMINAL -e ranger --choosefile=/tmp/ranger/sentfile && kdeconnect-cli --share "$(cat /tmp/ranger/sentfile)" -d $2 
	    fi;;
	*'Unpair' ) kdeconnect-cli --unpair -d $2 ;;
	*'Send SMS' ) 
	    message=$(echo 'OTW' | rofi -dmenu -i -p "Msg to send")
	    recipient=$(echo '14039199518' | rofi -dmenu -i -p "Recipient's phone #")
...
show_pmenu () {
    menu="$(printf "Pair Device" | rofi -dmenu -i -p "$1"  )"
    case "$menu" in
